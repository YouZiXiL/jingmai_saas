...
class CertificateDownloader
{
    ...
    private function job(array $opts): void
    {
        ...
        $stack->before('verifier', Middleware::mapResponse(self::certsRecorder((string) $outputDir, $certs,$opts['mchid'])), 'recorder');
     ...
     * @return callable(ResponseInterface)
     */
    private static function certsRecorder(string $outputDir, array &$certs,$mchid=null): callable {
        return static function(ResponseInterface $response) use ($outputDir, &$certs,$mchid): ResponseInterface {
            $body = (string) $response->getBody();
            /** @var object{data:array<object{effective_time:string,expire_time:string:serial_no:string}>} $json */
            $json = \json_decode($body);
            $data = \is_object($json) && isset($json->data) && \is_array($json->data) ? $json->data : [];
            \array_walk($data, static function($row, $index, $certs) use ($outputDir,$mchid) {
                $serialNo = $row->serial_no;
                $outpath = $outputDir . \DIRECTORY_SEPARATOR . $mchid . '.pem';
