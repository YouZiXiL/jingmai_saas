...
class Config extends Backend
{
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($row['wx_mchid']) || !empty($row['wx_platformcertificate']) || !empty($row['wx_mchcertificateserial']) || !empty($row['wx_mchprivatekey'])) {
                if (strlen($row['wx_mchprivatekey']) != 32) {
                ...
                $signContent = "GET\n/v3/certificates\n" . $timestamp . "\n" . $nonce_str . "\n\n";
                // 解析 key 供其他函数使用。
                openssl_sign($signContent, $sign, $row['wx_platformcertificate'], "SHA256");
                $signature = base64_encode($sign);
                $curl_v = curl_version();
                // 含有服务器用于验证商户身份的凭证
                $authorization = 'WECHATPAY2-SHA256-RSA2048 mchid="' . $row['wx_mchid'] . '",nonce_str="' . $nonce_str . '",signature="' . $signature . '",timestamp="' . $timestamp . '",serial_no="' . $row['wx_mchcertificateserial'] . '"';
                $header = [
                ...
                $plaintext = sodium_crypto_aead_aes256gcm_decrypt(base64_decode($ciphertext), $associated_data, $nonce, $row['wx_mchprivatekey']);
                $row['wx_serial_no']=$result['data'][0]['serial_no'];
                file_put_contents('uploads/apiclient_key/' . $row['wx_mchid'] . '.pem', $row['wx_platformcertificate']);
                file_put_contents('uploads/platform_key/' . $row['wx_mchid'] . '.pem', $plaintext);
            }
