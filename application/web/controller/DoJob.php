        ...
        try {
            ...
            if ($data['type']==1){
                ...
                if ($row['pay_status']!=2){
                    ...
                    Log::info("退款单号：{$out_refund_no}");
                    Log::info("超重状态：{$row['overload_status']}");
                    //超重退款
                    ...
                    Log::info("耗材状态：{$row['consume_status']}");
                    //耗材退款
                    ...
                    Log::info("更新订单");
                    $r = db('orders')->where('id',$data['order_id'])->update($up_data);
                    Log::info("更新结果：{$r}");
                }
            }
            return true;
        }catch (\Exception $e){
            Log::log(['队列执行异常' =>$e->getMessage()]);
            return false;
        }
